...
const objLat = (obj) => {
    ...
    let remainingLetters = fName.slice(1);
    let firstNameCap = firstLetterCap+remainingLetters;

    let lName=obj.lastName;
    let lastLetter = lName.charAt(0);
    let lastLetterCap = lastLetter.toUpperCase();
    let lastRemainingLetters = lName.slice(1);
    let lastNameCap = lastLetterCap+lastRemainingLetters;
...
const cvFormatter = (arr) => {
   ...
   for (let i=0;i<arr.length;i++){
    ...
    }else if (arr[i].firstName !== null) {fName = arr[i].firstName}
    else if (arr[i].lastName !== null) {fName = arr[i].lastName}
        let obj = {fullName : fName, tech : arr[i].tech}
        cvMatters.push(obj)
    }
...
const applicationsStatics = (arr) => {
  ...
  result.rejectedApplicants = arr.length - result.python_devs - result.javaScript_devs - result.dotNet_devs - result.java_devs;
  return result;
};
// -------------------------------------------------------------------------------------------------------
